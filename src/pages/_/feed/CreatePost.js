...
const CreatePost = ({ toggle, isOpen }) => {
    ...
    validate: (values) => {
      ...
      if (validationErrors)
        validationErrors.forEach(
          (err) => (errors[err.context.label] = err.message)
        );

      if (errors.location && !(errors.longitude && errors.latitude))
  ...
  return (
    <>
      <Modal
        isOpen={isOpen}
        toggle={() => toggleModal(formik.resetForm)}
        centered
      >
        ...
        <form onSubmit={formik.handleSubmit}>
          <ModalBody>
            ...
            <FormGroup>
              ...
              <Select
                ...
                onChange={(crimeCategory) =>
                  formik.setFieldValue("crimeCategory", crimeCategory.value)
                }
              />
            </FormGroup>
            <FormGroup>
            ...
            <FormGroup>
              ...
              {formik.values.image.length < 1 ? (
                <Alert color="danger">Please insert Image</Alert>
              ) : null}
            </FormGroup>
            <FormGroup
              style={{
          ...
          <ModalFooter>
            <Button
              color="light"
              size="sm"
              onClick={(e) => toggleModal(formik.resetForm)}
            >
              Cancel
            </Button>
            <Button
