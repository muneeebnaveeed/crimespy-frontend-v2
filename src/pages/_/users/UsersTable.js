...
function UsersTable(props) {
    ...
    const handleChangeRole = useCallback(async (role, id) => {
        setChangingRole(id);
        try {
            await db.collection("users").doc(id).update({role: role.value.title, permissions: role.value.permissions});
        ...
        <>
            <Card> {
                ...
                ((users.isError && ! users.isLoading) || (presets.isError && ! presets.isLoading)) && (
                    <Error for={
                            users.isError ? "users" : "roles"
                        }
                        onClick={
                            users.isError ? users.refetch : presets.refetch
                        }/>
                )
            }
                                        ...
                                        <tr key={i}>
                                            ...
                                            <Th>
                                                <Select // as={AsyncSelect}
                                                    ...
                                                    onChange={
                                                        (role) => role.label !== user.role ? handleChangeRole(role, user.uid) : null
                                                    }
            ...
            <Modal isOpen={show}
                ...
                <ModalFooter>
                    <Button color="light" w="55.5px" size="sm"
                        onClick={
                            () => setShow(!show)
                    }>
                        No
                    </Button>
                    <Button w="55.5px" color="primary" size="sm"
                        onClick={
                            () => deleteUser(userid)
                        }
